...
for num in mylist:
	cluster_centers.append((xy2index[num][0],xy2index[num][1]))
...
for j in range(10):
	...
	for item in xy2index:
		...
		if indice not in cluster_table.keys():
			cluster_table[indice] = [item]
		else:
			temp = cluster_table[indice]
			temp.append(item)
			cluster_table[indice] = temp
